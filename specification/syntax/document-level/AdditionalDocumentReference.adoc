...
|`cbc:UUID`
|**Referenced document UUID** +
The UUID assigned to the referenced or attached document, separate from its identifier in `cbc:ID` and from the UUID of the current Invoice or CreditNote. +
Provide the value directly in this element.
