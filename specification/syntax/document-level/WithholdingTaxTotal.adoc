`cac:WithholdingTaxTotal` is available at document (header) level on both `Invoice` and `CreditNote`, with cardinality `0..n`. It must appear after all `cac:TaxTotal` elements and before `cac:LegalMonetaryTotal`. This section describes header-level withholding only. Line-level withholding remains available on `InvoiceLine`; `CreditNoteLine` does not support `cac:WithholdingTaxTotal`.

The table below shows the available fields in `cac:WithholdingTaxTotal/cac:TaxSubtotal`. Additional elements are also available through the extension described in <<_taxsubtotal, cac:WithholdingTaxTotal/cac:TaxSubtotal>>.
...
*Examples* +
_Header-level cac:WithholdingTaxTotal on an Invoice_
[source,xml]
----
<Invoice>
...
_Header-level cac:WithholdingTaxTotal on a CreditNote_
[source,xml]
----
<CreditNote>
  <!-- Code omitted for clarity -->
