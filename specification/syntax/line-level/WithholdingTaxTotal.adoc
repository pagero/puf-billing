`cac:WithholdingTaxTotal` is available at line level on both `InvoiceLine` and `CreditNoteLine`, with cardinality `0..n`. It must appear after all `cac:TaxTotal` elements on the line.

.Elements added in cac:InvoiceLine/cac:WithholdingTaxTotal and cac:CreditNoteLine/cac:WithholdingTaxTotal
|===
|Path |Description
